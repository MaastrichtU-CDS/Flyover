...
from modules.database_operations import (
    check_graph_exists, execute_query, retrieve_categories, 
    get_column_class_uri, get_existing_column_class_uri
)
from modules.file_operations import allowed_file, retrieve_global_names
from modules.data_processing import (
    formulate_local_semantic_map, handle_postgres_data, insert_equivalencies
)
from modules.graph_operations import (
    background_pk_fk_processing, background_cross_graph_processing
)
from modules.triplifier_operations import run_triplifier
    ...
    if file_type == 'CSV' and csv_files:
        ...
        try:
            success, message = run_triplifier(
                ...
                session_cache.pk_fk_data,
                session_cache.cross_graph_link_data,
                background_pk_fk_wrapper,
                background_cross_graph_wrapper
            )
        finally:
        ...
        connection_result = handle_postgres_data(request.form.get('username'), request.form.get('password'),
                             request.form.get('POSTGRES_URL'), request.form.get('POSTGRES_DB'),
                             request.form.get('table'), root_dir, child_dir)
        if connection_result is not None and hasattr(connection_result, 'data'):
            # handle_postgres_data returned a template render, which means there was an error
            return connection_result
        
        # Store the connection for the session cache
        session_cache.conn = connection_result
        success, message = run_triplifier(
...
def describe_variables():
    ...
    column_info = pd.read_csv(StringIO(execute_query(session_cache.repo, column_query, graphdb_url)))
    ...
    global_names = retrieve_global_names(session_cache.global_semantic_map)
        ...
        for local_variable_name in request.form:
            if (not re.search("^ncit_comment_", local_variable_name) and
                ...
                if data_type == 'Categorical':
                    cat = retrieve_categories(session_cache.repo, local_variable_name, graphdb_url)
                ...
                # add the local variable to a list of variables to further specify
                    ...
                    def execute_query_wrapper(repo, query, query_type="query", endpoint_appendices=""):
                        return execute_query(repo, query, graphdb_url, query_type, endpoint_appendices)
                    insert_equivalencies(session_cache.descriptive_info[database], local_variable_name, session_cache.repo, execute_query_wrapper)
...
def retrieve_detailed_descriptive_info():
    ...
    for database in session_cache.databases:
        ...
        for variable in set(variables):
            ...
            def execute_query_wrapper(repo, query, query_type="query", endpoint_appendices=""):
                return execute_query(repo, query, graphdb_url, query_type, endpoint_appendices)
            insert_equivalencies(session_cache.descriptive_info[database], variable, session_cache.repo, execute_query_wrapper)
        ...
        if len(session_cache.databases) > 1:
            ...
            for database in session_cache.databases:
                ...
                with zipfile.ZipFile(_filename, 'a') as zipf:
                    ...
                    modified_semantic_map = formulate_local_semantic_map(database, session_cache.global_semantic_map, session_cache.descriptive_info)
...
def annotation_review():
    ...
    for database in session_cache.databases:
        ...
        local_semantic_map = formulate_local_semantic_map(database, session_cache.global_semantic_map, session_cache.descriptive_info)
...
def formulate_local_semantic_map(database, session_cache.global_semantic_map, session_cache.descriptive_info):
    """
...
def insert_equivalencies(descriptive_info, variable):
    ...
    query = f"""
            ...
            """
    return execute_query(session_cache.repo, query, graphdb_url, "update", "/statements")
...
def get_column_class_uri(table_name, column_name):
    ...
    try:
        query_result = execute_query(session_cache.repo, query, graphdb_url)
...
def insert_fk_relation(fk_predicate, column_class_uri, target_class_uri):
    ...
    return execute_query(session_cache.repo, insert_query, graphdb_url, "update", "/statements")
...
def get_existing_graph_structure():
    ...
    try:
        ...
        result = execute_query(session_cache.repo, structure_query, graphdb_url)
...
def background_pk_fk_wrapper():
    """Wrapper function for background PK/FK processing"""
    def pk_fk_status_callback(status):
        session_cache.pk_fk_status = status
        
    def get_column_class_uri_wrapper(table_name, column_name):
        return get_column_class_uri(table_name, column_name, session_cache.repo, graphdb_url)
    ...
    background_pk_fk_processing(
        session_cache.pk_fk_data, 
        session_cache.repo,
        get_column_class_uri_wrapper,
        execute_query_wrapper,
        pk_fk_status_callback
    )


def background_cross_graph_wrapper():
    """Wrapper function for background cross-graph processing"""
    def cross_graph_status_callback(status):
        session_cache.cross_graph_link_status = status
        
    def get_column_class_uri_wrapper(table_name, column_name):
        return get_column_class_uri(table_name, column_name, session_cache.repo, graphdb_url)
        
    def get_existing_column_class_uri_wrapper(table_name, column_name):
        return get_existing_column_class_uri(table_name, column_name, session_cache.repo, graphdb_url)
    ...
    background_cross_graph_processing(
        ...
        session_cache.repo,
        get_column_class_uri_wrapper,
        get_existing_column_class_uri_wrapper,
        execute_query_wrapper,
        cross_graph_status_callback
    )
