"""
Backend unit: the Ollama LLM client.
...
class TestMatchEquivalents(unittest.TestCase):
    ...
    def setUp(self):
        self.client = OllamaClient("http://ollama:11434")

    @patch("services.llm.ollama_client.requests.post")
    def test_sends_schema_constrained_payload(self, mock_post):
        ...
        self.client.match_equivalents(["age"], ["patient_age"], model="m")
    ...
    def test_clamps_confidence(self, mock_post):
        ...
        result = self.client.match_equivalents(
            ["age", "sex"], ["patient_age", "biological_sex"], model="m"
        )
    ...
    def test_nulls_hallucinated_match(self, mock_post):
        ...
        result = self.client.match_equivalents(["age"], ["patient_age"], model="m")
    ...
    def test_nulls_duplicate_match_first_wins(self, mock_post):
        ...
        result = self.client.match_equivalents(
            ["age", "leeftijd"], ["patient_age"], model="m"
        )
    ...
    def test_preserves_list_a_order(self, mock_post):
        ...
        result = self.client.match_equivalents(["a_col", "b_col"], ["x"], model="m")
    ...
    def test_empty_content_raises(self, mock_post):
        ...
        with self.assertRaises(OllamaError):
            self.client.match_equivalents(["age"], ["x"], model="m")
...
class TestEnsureModel(unittest.TestCase):
    ...
    @patch("services.llm.ollama_client.requests.get")
    ...
    @patch.object(OllamaClient, "has_model", return_value=True)
    @patch.object(OllamaClient, "is_reachable", return_value=True)
    ...
    @patch.object(OllamaClient, "has_model")
    @patch.object(OllamaClient, "is_reachable", return_value=True)
    ...
    @patch.object(OllamaClient, "pull_model", side_effect=OllamaError("fail"))
    @patch.object(OllamaClient, "has_model", return_value=False)
    @patch.object(OllamaClient, "is_reachable", return_value=True)
