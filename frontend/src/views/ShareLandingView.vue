...
async function downloadSemanticMap() {
  try {
    // The backend's /downloadSemanticMap reads from server-side session
    // state which is empty in the SPA happy path (the map lives in
    // IndexedDB). Prefer the IndexedDB copy when present so users can
    // download a filled-in map they only ever edited client-side.
    const local = await db.getData('metadata', 'semantic_map')
    if (local?.data) {
      const blob = new Blob([JSON.stringify(local.data, null, 2)], {
...
onMounted(async () => {
  try {
    const local = await db.getData('metadata', 'semantic_map')
    hasSemanticMap.value = !!local?.data
  } catch {
