...
function stripOtherFromLocalMappings(data) {
  // "Other" is a UI placeholder selectable on Describe Variable Details. It
  // must not be sent to backend validation: there's no schema term keyed
  // "other", so the validator rejects the whole map. Strip any leftover
  // entries before submitting (covers maps written by older builds).
  if (!data?.databases) return data
  for (const db of Object.values(data.databases)) {
    if (!db?.tables) continue
    for (const table of Object.values(db.tables)) {
      if (!table?.columns) continue
      for (const col of Object.values(table.columns)) {
        if (col?.localMappings && 'other' in col.localMappings) {
          delete col.localMappings.other
        }
      }
    }
  }
...
async function startAnnotationProcess() {
  ...
  try {
    const payload = stripOtherFromLocalMappings(
      JSON.parse(JSON.stringify(semanticMapData.value))
    )
