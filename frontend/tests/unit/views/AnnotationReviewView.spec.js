...
describe('Frontend unit: AnnotationReviewView', () => {
  beforeEach(() => {
    api.get.mockReset()
  ...
  it('strips leftover localMappings.other from older maps before POSTing', async () => {
    // Defense-in-depth for the UI "Other" placeholder bug: maps written by
    // older builds may carry localMappings.other, which backend validation
    // rejects. The submit path must scrub it.
    const polluted = JSON.parse(JSON.stringify(SEMANTIC_MAP))
    polluted.databases.db_a.tables.patients.columns.age_col.localMappings = {
      yes: ['Y'],
      other: ['Z'],
    }
    api.get.mockResolvedValue({ data: { success: true, databases: ['patients.csv'] } })
    db.getData.mockImplementation(async (_store, key) => {
      if (key === 'semantic_map') return { data: polluted }
      return null
    })
    fetch.mockResolvedValueOnce(jsonResponse({ success: true }))
    fetch.mockResolvedValueOnce(jsonResponse({ success: true }))
    ...
    const col = submitted.databases.db_a.tables.patients.columns.age_col
    expect(col.localMappings).toEqual({ yes: ['Y'] })
